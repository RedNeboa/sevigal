...
class pack_line(models.Model):
    ...
    def get_name_quantity(self):
        #pydevd.settrace("10.0.3.1")
        if self.product_id:
            self.name_quantity = "%s: %d" % (self.product_id.name, self.quantity)
        else:
            self.name_quantity = ''
...
class contrato(models.Model):
    ...
    def obtener_line_factu_recurr_no_pack(self, producto, nueva=False):
        ...
        '''
        lineas_contrato = self.env['account.analytic.invoice.line'].search([
    ...
    '''
    def crear_oportunidad_agotamiento_pack(self, partner, producto):
        '''
