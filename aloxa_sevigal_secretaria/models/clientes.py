...
class res_partner(models.Model):
    ...
    llamada_realizada_ids = fields.One2many(
        'sevigal.llamada.realizada', 'partner_id', 'Llamadas Realizadas')
    fax2mail_ids = fields.One2many(
        'sevigal.fax2mail', 'partner_id', 'Llamadas Realizadas')
    servicio_exclusivo_ids = fields.One2many(
        'sevigal.servicio.exclusivo', 'partner_id', 'Servicios Exclusivos')
    contrato_id = fields.Many2one(
        'account.analytic.account', 'Contrato', compute=get_contrato_cliente)
    ...
    def crear_usuario(self):
        ...
        if self.name and self.email:
            ...
            if extid_portal_group:
                portal_group = self.env['res.groups'].search([
                    ('id', '=', extid_portal_group.res_id)
                ])
            ...
            if extid_partner_group:
                partner_group = self.env['res.groups'].search([
                    ('id', '=', extid_partner_group.res_id)
                ])
            ...
            if extid_base_user_group:
                base_user_group = self.env['res.groups'].search([
                    ('id', '=', extid_base_user_group.res_id)
                ])
...
class protocolos_cliente(models.Model):
    ...
    protocolo = fields.Selection([
        ('Transferir_si', 'Transferir'),
        ('Email_si', 'Email'),
