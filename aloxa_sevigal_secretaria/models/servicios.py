    ...
    def enviar_email(self, modelo=''):
        for res in self:
            ...
            return  {
                ...
                'context': {
                    'servicio_id': res.id,
                    'modelo': 'sevigal.llamada.realizada',
...
class fax2mail(models.Model):
    ...
    '''
    Para los Fax2Mails se da un comportamiento hibrido entre registro de llamada
    y computo de servicio. En este metodo se localiza linea de pack para Fax2Mail
    y se imputa el consumo, creando linea adicional cuando se agote
    '''
    @api.multi
    def registrar_consumo_servicio(self):
        #pydevd.settrace("192.168.3.1")
                                ...
                                if pcline.product_id and pcline.product_id.tipo_producto_pack:
                                    if pcline.product_id.tipo_producto_pack == 'Fax2Mail':
                                        ...
                                        if ctdad_producto_consumido < ctdad_producto_pack:
                                            ...
                                            el numero de unidades de aviso. En
                                            caso afirmativo se creara una
                                            oportunidad de ampliacion de pack
                                            para ese cliente
                                            '''
                                            ...
                                            if opciones:
                                                ...
                                                if ctdad_producto_pack - ctdad_producto_consumido == uni_aviso:
                                                    contrato.crear_oportunidad_agotamiento_pack(
                                                        self.partner_id, pcline.product_id)
