...
class product_product(models.Model):
    ...
    #Fields
    tipo_producto_pack = fields.Selection([
        ('Llamada', 'Llamada'),
        ('SMS', 'SMS'),
        ('Email', 'Email'),
        ('Apunte Agenda', 'Apunte Agenda'),
        ('Fax2Mail', 'Fax2Mail'),
    ], 'Tipo de producto en Pack')
    tipo_producto_servicio = fields.Selection([
        ('Llamada Transferida', 'Llamada Transferida'),
    ...
    pack_price_type = fields.Selection([
        ('Detailed - Fixed Price', 'Detailed - Fixed Price'),
...
class product_pack_contrato(models.Model):
    ...
    def get_name_quantity(self):
        #pydevd.settrace("10.0.3.1")
        if self.product_id:
            self.name_quantity = "%s: %d de %d" % (self.product_id.name,
                                                   self.consumido,
                                                   self.disponible)
        else:
            self.name_quantity = ''
